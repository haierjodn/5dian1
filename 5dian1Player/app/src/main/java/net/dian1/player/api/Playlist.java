...
public class Playlist implements Serializable {
	...
	/**
	 * Add single music to the playlist
	 * 
	 * @param music
	 *            <code>Music</code> instance
	 * @param album
	 *            <code>Album</code> instance
	 */
	public void addTrack(Music music, Album album) {
		PlaylistEntry playlistEntry = new PlaylistEntry();
		playlistEntry.setAlbum(album);
		playlistEntry.setMusic(music);
	...
	public void selectOrAdd(Music music, Album album) {

		// first search thru available tracks
		for (int i = 0; i < playlist.size(); i++) {
			if (playlist.get(i).getMusic().getId() == music.getId()) {
				select(i);
				return;
			}
		}

		// add music if necessary
		addTrack(music, album);
