...
import net.dian1.player.dialog.CustomEqualizer;
...
public class EqualizerActivity extends Activity{
    ...
    private void setupEqualizerFxAndUI() {
        ...
        Log.i(Dian1Application.TAG, "setupEqualizerFxAndUI " + equalizer.getNumberOfPresets());
        Settings settings = Dian1Application.getInstance().getEqualizerSettigns();
    	...
    	this.mRadioGroup.setOnCheckedChangeListener(new OnCheckedChangeListener(){
			public void onCheckedChanged(RadioGroup radioGroup, int checkedId) {
				...
				if (preset != null) {
					...
					if (Dian1Application.getInstance().isEqualizerRunning()) {
						final Equalizer eq = Dian1Application.getInstance().getMyEqualizer();
						eq.usePreset(preset);
						Dian1Application.getInstance().updateEqualizerSettings(eq.getProperties());
					} else {
						// save for later use
						Dian1Application.getInstance().setEqualizerPreset(preset);
					}
				}
			}
    	});
