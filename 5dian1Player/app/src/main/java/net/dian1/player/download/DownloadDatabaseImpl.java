...
public class DownloadDatabaseImpl implements DownloadDatabase {
	...
	private SQLiteDatabase getDb() {
		...
		if (success) {
			Log.i(Dian1Application.TAG, "Directory: " + "/sdcard/music"
					+ " created");
		}
		try {
			...
			Log.e(Dian1Application.TAG, "Failed creating database");
			e.printStackTrace();
			return null;
		}
	}
	...
	public void setStatus(PlaylistEntry entry, boolean downloaded) {
		...
		if (row_count == 0) {
			Log.e(Dian1Application.TAG, "Failed to update " + TABLE_LIBRARY);
		}
	}
	...
	public boolean trackAvailable(Music music) {
		...
		String[] selectionArgs = { "" + music.getId() };
	...
	public void remove(DownloadJob job) {
		...
		String[] whereArgs = { "" + job.getPlaylistEntry().getMusic().getId() };
	...
	private class DatabaseUpdaterV1 extends DatabaseUpdater {
		...
		public void update() {
			...
			try{
				...
				Log.v(Dian1Application.TAG, "Library table not existing");
			}
			createTables();
