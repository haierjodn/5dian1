...
import net.dian1.player.widget.AlbumBar;
...
public class PlaylistActivity extends Activity {
	...
	private OnItemClickListener mOnTracksItemClickListener = new OnItemClickListener(){
		...
		public void onItemClick(AdapterView<?> adapterView, View view, int index,
			...
			Dian1Application.getInstance().getPlayerEngineInterface().openPlaylist(mPlaylist);
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case CONTEXT_DOWNLOAD:
			Dian1Application.getInstance().getDownloadManager().download(mPlaylist.getTrack(menuInfo.position));
			break;
	...
	private void loadTracks(){
		...
		if(mPlaylist == null){
			mPlaylist = Dian1Application.getInstance().getPlayerEngineInterface().getPlaylist();
		}
